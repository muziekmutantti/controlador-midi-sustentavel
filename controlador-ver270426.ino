...
#define ATMEGA32U4 1  // * coloque aqui o uC que você está usando, como nas linhas acima seguidas de "1", como "ATMEGA32U4 1" "ATMEGA328 1", "DEBUG 1", etc.
/////////////////////////////////////////////
// BIBLIOTECAS
...
// debounce
unsigned long lastDebounceTime[N_BUTTONS] = {};  // a última vez que o pino de saída foi alternado
unsigned long debounceDelay = 30;                 //* o tempo de debounce; aumentar se a saída estiver mandando muitas notas de uma vez so
...
// LOOP
void loop() {
  // changeOctave();  // Atualiza a oitava quando os botões são pressionados
  botoes();
  potenciometros();
}
...
#elif DEBUG
          Serial.print(i);
          Serial.print(": Botão Ligado | ch:");
          Serial.println(midiCh);
          digitalWrite(LED, HIGH);  // turn the LED on (HIGH is the voltage level)
#endif

        } else {
// Envia a nota MIDI OFF de acordo com a placa escolhida e pisca o led
#ifdef ATMEGA328
          // ATmega328 (uno, mega, nano...)
